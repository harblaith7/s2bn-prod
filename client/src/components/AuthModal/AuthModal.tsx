...
function TransitionsModal(props: any) {
  ...
  return (
    <div className='Header__modal-container'>
      <button
        type='button'
        className='Header__about-btn Header__about-btn-modal'
        onClick={handleOpen}
      >
        Login In
      </button>
          ...
          <div className='AuthModal'>
            {isLogin ? (
              <>
                ...
                <LoginForm switchStates={setIsLogin} />
              </>
            ) : (
              <>
                {props.alert && <Alert alertType={1} />}
                <h2 className='AuthModal__title'>Sign Up</h2>
                <p className='AuthModal__blurb'>
                  Only administrator can sign up
                </p>
                <SignUpForm switchStates={setIsLogin} />
              </>
            )}
          </div>
        </Fade>
