...
class EventPage extends Component<IProps> {
  displayPage: () => JSX.Element = () => {
    ...
    if (!event) {
      return (
        <div className='EventErrorPage'>Opps, we can't find that event</div>
      );
    }
    ...
    return (
      <>
        ...
        />
        <AboutEvent longDescription={event.longDescription} />
      </>
    );
  };

  render() {
    return <div>{this.props.events.length ? this.displayPage() : ""}</div>;
  }
}
